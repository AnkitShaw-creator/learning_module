...
exports.getTopics = async (req, res) => {
    try {
        ...
        const query = "select * from topics t"; // selecting all the users from database
        ...
        Database.connect((err) => { // connecting to the database
            if (err)
                ...
                console.log("Connection successful: admin-getTopics");
        });
...
exports.getCourseOptions = async (req, res) => {
    try{
        ...
        /** query for the authenticating the user and retriving */
        const query = "select distinct courseCode, courseName from courses c ; "; // selecting all the users from database
        ...
        Database.connect((err) => { // connecting to the database
            if (err)
                ...
                console.log("Connection successful: admin-getCourseOptions");
        });
        Database.query(query, [], (err, data) => {
            if (err) {
                res.status(500).json(err)
